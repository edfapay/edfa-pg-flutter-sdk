
import 'package:edfapg_sdk/src/adapters/callbacks/TransactionStatusResponseCallback.dart';
import 'package:edfapg_sdk/src/response/base/error/Error.dart';

import 'TransactionStatusSuccess.dart';

class EdfaPgTransactionStatusResult{
  EdfaPgTransactionStatusSuccess? success;
  EdfaPgError? error;
  dynamic failure;
  Map? responseJSON;

  EdfaPgTransactionStatusResult(Map result){
    if(result.containsKey("success")) {
      success = EdfaPgTransactionStatusSuccess.fromJson(result["success"]);
    }

    if(result.containsKey("failure")) {
      failure = result["failure"];
    }

    if(result.containsKey("responseJSON")) {
      responseJSON = result["responseJSON"];
    }
  }

  triggerCallbacks(TransactionStatusResponseCallback? callback, {Function(dynamic)? onFailure, Function(Map)? onResponseJSON}){
    if(success != null) {
      callback?.success(success!);
    }

    if(error != null) {
      callback?.error(error!);
    }

    if(failure != null && onFailure != null) {
      onFailure(failure);
    }

    if(responseJSON != null && onResponseJSON != null) {
      onResponseJSON(responseJSON ?? {});
    }
  }
}
